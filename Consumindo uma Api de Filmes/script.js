const apiKey = '9fc509eb640458859596551bdc15cdc7';
...
let favorites = localStorage.getItem("filmes") ? JSON.parse(localStorage.getItem("filmes")) : [];
    ...
    fetch(`${apiUrl}movie/${movieId}?api_key=${apiKey}&language=${language}`)
        ...
        .then(data => {
            ...
            movieDetails.innerHTML = `
                ...
                <button onclick="getRelatedMovies(${movieId})">Outros</button>
            `;
...
function toggleFavorite(movieId) {
    ...
    if (movieIndex === -1) {
        ...
        localStorage.setItem("filmes", JSON.stringify(favorites))
    } else {
...
// Função para exibir os filmes na aba de favoritos
function displayMoviesFavoritos() {
    favoriteList.innerHTML = ''; // Limpa a lista de filmes
    favorites.forEach(movie => {
        fetch(`${apiUrl}movie/${movie}?api_key=${apiKey}&language=${language}`)
            .then(response => response.json())
            .then(data => {
                const movieCard = document.createElement('div');
                movieCard.classList.add('movie-card');
                movieCard.innerHTML = `
                    <img src="https://image.tmdb.org/t/p/w200${data.poster_path}" alt="${data.title}">
                    <h3>${data.title}</h3>
                    <button onclick="toggleFavoriteFavorito(${data.id})">Remover</button>
                `;
...
function toggleFavoriteFavorito(movieId) {
    const movieIndex = favorites.indexOf(movieId);
    if (movieIndex === -1) {
        favorites.push(movieId);
        localStorage.setItem("filmes", JSON.stringify(favorites))
...
movieSearch.addEventListener("keydown", (e) => {
    if (e.keyCode == 13) {
        const val = movieSearch.value
        searchMovies(val)
    }
})

telaPreta.addEventListener("click", () => {
    movieDetails.style.display = "none"
    telaPreta.style.display = "none"
})